...
void transferer_elements_et_libere(
	Ensemble * destination, Ensemble * source 
){
	ajouter_elements( destination, source );
	liberer_ensemble( source );
}

void transferer_elements(
	Ensemble * destination, Ensemble * source 
){
	ajouter_elements( destination, source );
	vider_ensemble( source );
}
...
Ensemble_iterateur trouver_ensemble(	const Ensemble* ensemble, const intptr_t element){
