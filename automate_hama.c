...
Automate * creer_union_des_automates(	const Automate * automate_1, const Automate * automate_2){
  //	A_FAIRE_RETURN( NULL );
       	Automate * res = creer_automate(); 
        Automate * copie_de_automate_1 =copier_automate( automate_1);
...
Automate *automate_accessible( const Automate * automate ){
  ...
  /* Ensemble * res = accessibles(automate); */
  /* Automate * automate1 = creer_automate(); */
  /* Ensemble * finaux1 = creer_intersection_ensemble( res, get_finaux(automate) ); */
 
  /* automate1->initiaux = automate->initiaux; */
  /* automate1->finaux = finaux1; */
  /* automate1->etats = res; */
  /* automate1->alphabet = automate->alphabet; */
  ...
  /*       Cle * cle = (Cle*) get_cle( it2 ); */
  /*       Ensemble * fins = (Ensemble*) get_valeur( it2 ); */
  /*       if(est_un_etat_de_l_automate( automate1,cle->origine)  && est_un_etat_de_l_automate( automate1 ,get_valeur( it2 )) ){ */
  ...
  /*       ){ */
  /*           int fin = get_element( it1 ); */
  /*           ajouter_transition(automate1,  cle->origine ,cle->lettre, fin); */
  /*       } */
  /*           } */
  /*   } */
...
Automate * creer_automate_du_melange(
  ...
  Automate * new_automate_2 =  translater_automate(automate_2 ,automate_1);
  Automate * new_automate_1 = copier_automate(automate_1);
  Automate * new_res =creer_automate(); 
  new_res =  creer_union_des_automates(new_automate_1 , new_automate_2);
 

/* // On ajoute les transitions */
 	Table_iterateur it2 ;  
         Ensemble_iterateur it1; 
         Ensemble_iterateur it3 , it4; 
         int init ;  


	
        Ensemble * embl = new_automate_2->initiaux;
	Ensemble * embl_fin = new_automate_1->finaux;
	for(    
                it4 = premier_iterateur_ensemble( embl );
		! iterateur_est_vide( it4 );
		it4 = iterateur_suivant_ensemble( it4 )
){
	  
       for(
		it2 = premier_iterateur_table(new_automate_2->transitions );
		! iterateur_est_vide( it2 );
		it2 = iterateur_suivant_table( it2 )
	){
		Cle * cle = (Cle*) get_cle( it2 );
		Ensemble * fins = (Ensemble*) get_valeur( it2 );
		init = get_element(it4);
		if(init == cle->origine){
                   for(it3= premier_iterateur_ensemble( embl_fin );
			! iterateur_ensemble_est_vide( it3 );
			it3 = iterateur_suivant_ensemble( it3 ) )
		    {
 	 	       int debut = get_element(it3); 
 		for(
			it1 = premier_iterateur_ensemble( fins );
			! iterateur_ensemble_est_vide( it1 );
			it1 = iterateur_suivant_ensemble( it1 )
			...
			int fin = get_element( it1 );
			ajouter_transition( new_res, debut, cle->lettre, fin );
		}
           }
	}
       }
        ...
        Ensemble * embl2 = new_automate_1->initiaux;
	Ensemble * embl_fin2 = new_automate_2->finaux;

for(    
                it4 = premier_iterateur_ensemble( embl2 );
		! iterateur_est_vide( it4 );
		it4 = iterateur_suivant_ensemble( it4 )
){
	  
       for(
		it2 = premier_iterateur_table(new_automate_1->transitions );
		! iterateur_est_vide( it2 );
		it2 = iterateur_suivant_table( it2 )
	){
		Cle * cle = (Cle*) get_cle( it2 );
		...
		init = get_element(it4);
		if(init == cle->origine){
                   for(it3= premier_iterateur_ensemble( embl_fin2 );
			! iterateur_ensemble_est_vide( it3 );
			it3 = iterateur_suivant_ensemble( it3 ) )
		    {
 	 	       int debut = get_element(it3); 
